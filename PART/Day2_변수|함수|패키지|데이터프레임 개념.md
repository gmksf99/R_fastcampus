...
2. Column이 많다 -> 분석방법의 한계 -> 고급 분석방법(machin learning)
...
`d <- c(1:5)`
`e <- seq(1,5)`
모두 변수에 1, 2, 3, 4, 5가 저장된다.
`f <- seq(1,10, by=2)` : 1~10까지 2씩 증가
	* 연산
	   `c+2` : 결과 3, 4, 5, 6, 7
	   `c+d` : 결과 2, 4, 6, 8, 10	
	   <br/>
* **변수에 문자 저장**
`a2 <- "a"`  : 변수 a2에 "a"를 저장
`b2 <- "Hello world!"`  # 문자 및 특수기호를 저장
`c2 <- c("Hello!","world","is","good!")` # 연속된 문자를 저장
 > **사칙연산은 숫자값에만 적용가능!!**  
### **:bulb:함수**
* `a <- c(1,2,3)`
		`mean(a)` : 결과 2
